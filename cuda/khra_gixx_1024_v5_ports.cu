...
//          6=inject_density, 7=stress_snapshot_now, 0=handled/noop
...
int main() {
    ...
    snprintf(ep_snapshot,  sizeof(ep_snapshot),  "tcp://127.0.0.1:%d", port_base + 2);
    snprintf(ep_ack,       sizeof(ep_ack),       "tcp://127.0.0.1:%d", port_base + 3);
    snprintf(ep_stress,    sizeof(ep_stress),    "tcp://127.0.0.1:%d", port_base + 4);
    ...
    while (running) {
        ...
        if (cmd_len > 0) {
            ...
            if (cmd_result == 1) {
                ...
                zmq_send_resilient(&ack_pub, zmq_ctx, ep_ack,
        ...
        if (cycle % 10 == 0) {
            ...
            zmq_send_resilient(&publisher, zmq_ctx, ep_telemetry,
            ...
            if ((snapshot_interval > 0 && cycle > 0 && cycle % snapshot_interval == 0)
                ...
                // v4: resilient send with auto-reconnect
                zmq_send_resilient(&snapshot_pub, zmq_ctx, ep_snapshot,
            ...
            // Fires on stress_snapshot_now command — sends per-cell sxx/syy/sxy
            // Header: same 8-byte format (cycle u32, width u16, height u16)
            // Data: sxx[NX*NY] + syy[NX*NY] + sxy[NX*NY] as packed float32
            // h_sxx/h_syy/h_sxy are already on host from telemetry copy above
            if (stress_snapshot_now_flag) {
                ...
                // v4: resilient send with auto-reconnect
                zmq_send_resilient(&stress_pub, zmq_ctx, ep_stress,
